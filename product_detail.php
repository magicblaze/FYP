...
if (!empty($colorImages)) {
    ...
    if ($firstImage) $mainImg = 'uploads/products/' . htmlspecialchars($firstImage);
}
if (!$mainImg) $mainImg = 'uploads/products/placeholder.jpg';

function colorNameToHex($colorInput) { /* function body same as client version, omitted for brevity */
    $colorInput = trim($colorInput);
    if (preg_match('/^#[0-9A-Fa-f]{6}$/i', $colorInput)) return strtoupper($colorInput);
...
<head>
    ...
    <style>/* condensed styles omitted */</style>
</head>
<body>
            ...
            <div class="product-panel">
                ...
                <div class="product-stats">
                    <div class="likes-count">
                        <button class="heart-icon <?= $liked ? 'liked' : '' ?>" id="likeHeart" data-productid="<?= (int)$product['productid'] ?>" title="Like this product"><?= $liked ? '♥' : '♡' ?></button>
                ...
                <div class="color-selection-section">
                    ...
                    <?php if (count($colors) <= 5): ?>
                        <div class="color-options" id="colorOptions">
                            <?php foreach ($colors as $index => $color): $hexColor = colorNameToHex($color); ?>
                                <button type="button" class="color-button <?= $index === 0 ? 'selected' : '' ?>" data-color="<?= htmlspecialchars($color) ?>" data-hex="<?= htmlspecialchars($hexColor) ?>" onclick="selectColor(this)" title="<?= htmlspecialchars($color) ?>"><span class="color-swatch" style="background-color: <?= htmlspecialchars($hexColor) ?>;"></span></button>
                            <?php endforeach; ?>
                        </div>
                    <?php else: ?>
                        <select class="color-select-dropdown" id="colorSelect" onchange="selectColorDropdown(this)">
                            <option value="">-- Choose a color --</option>
                            <?php foreach ($colors as $color): $hexColor = colorNameToHex($color); ?>
                                <option value="<?= htmlspecialchars($color) ?>" data-hex="<?= htmlspecialchars($hexColor) ?>"><?= htmlspecialchars($color) ?></option>
                            <?php endforeach; ?>
                        </select>
                    <?php endif; ?>
    ...
    document.getElementById && document.getElementById('likeHeart') && document.getElementById('likeHeart').addEventListener('click', function(e){ e.preventDefault(); const productid = this.dataset.productid; const heart = this; const formData = new FormData(); formData.append('action','toggle_like'); formData.append('type','product'); formData.append('id',productid); fetch('api/handle_like.php',{ method:'POST', body: formData }).then(r=>r.json()).then(data=>{ if (data.success){ if (data.liked){ heart.classList.add('liked'); heart.textContent='♥'; } else { heart.classList.remove('liked'); heart.textContent='♡'; } document.getElementById('likeCount').textContent = data.likes; } else { alert('Error: '+(data.message||'Failed to update like')); } }).catch(err=>{ console.error(err); alert('An error occurred while updating the like.'); }); });
    </script>
