...
<body>
    ...
    <style>
        ...
        .stat-card.designing .stat-value {
            color: #17a2b8;
        }
    ...
    <body>
        ...
        <div class="page-container">
            <h1 class="page-title">Schedule Management - <?php echo htmlspecialchars($user_name); ?></h1 <!-- 过滤器 -->
            ...
            if (mysqli_num_rows($result) == 0) {
                ...
                <div>
                    <strong>No scheduled found' . (!empty($search) ? ' matching your search criteria.' : ' at the moment.') . '</strong>
                    <p class="mb-0">Order that have been approved will appear here with their schedule.</p>
                </div>
            </div>';
            } else {
                ...
                <div class="stats-grid no-print">
                    <div class="stat-card designing">
                        ...
                        <div style="font-size: 12px; color: #888; margin-top: 5px;">Active order being designed</div>
                    </div>
                    <div class="stat-card completed">
                        <div class="stat-value"><?php echo $stats['total_completed'] ?? 0; ?></div>
                        <div class="stat-label">Completed</div>
                        <div style="font-size: 12px; color: #888; margin-top: 5px;">Successfully delivered order</div>
                    </div>
                    ...
                    <div class="stat-card today">
                        ...
                        <div style="font-size: 12px; color: #888; margin-top: 5px;">Order due today</div>
                    </div>
                    <div class="stat-card">
                    ...
                    <div class="stat-card">
                        ...
                        <div style="font-size: 12px; color: #888; margin-top: 5px;">All scheduled order</div>
                    </div>
                </div>
                        ...
                        <tbody>
                            ...
                            while ($row = mysqli_fetch_assoc($result)):
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="d-flex flex-column">
                                            ...
                                            <small class="text-muted">Email:
                                                <?php echo htmlspecialchars($row["client_email"]); ?></small>
                                        </div>
                                    </td>
                                    <td>
                                        <span
                                            class="status-badge <?php echo in_array(strtolower($row["ostatus"]), ['completed', 'complete']) ? 'status-completed' : 'status-designing'; ?>">
                                    ...
                                    <td><strong
                                            class="text-success">HK$<?php echo isset($row["budget"]) ? number_format($row["budget"], 2) : '0.00'; ?></strong>
                                    </td>
                                    <td>
                                        <?php if (!empty($row["designid"])): ?>
                                            <small>Design #<?php echo htmlspecialchars($row["designid"]); ?></small><br>
                                            <small
                                                class="text-muted"><?php echo htmlspecialchars(substr($row["design_name"] ?? $row["design_tag"] ?? '', 0, 20)); ?></small>
                                        <?php else: ?>
                ...
                <div class="card mt-3">
                    <div class="card-body d-flex justify-between align-center">
                        <div>
                            <strong>Showing <?php echo mysqli_num_rows($result); ?> schedule</strong>
