...
if ($role === 'supplier') {
    ...
    if($cData) $introduction = $cData['introduction'];
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_profile'])) {
    ...
    if ($stmt->execute()) { $success = "Profile updated!"; header("Refresh:1"); }
}
...
<head>
    ...
    <style>
        ...
        .logo-box { width: 90px; height: 90px; border: 1px solid #eee; border-radius: 10px; display: flex; align-items: center; justify-content: center; margin-right: 25px; padding: 5px; background: #fff; }
        ...
        .content-section { background: #fff; border-radius: 12px; padding: 20px; margin-top: 20px; }
        .worker-card { background: #fff; border: 1px solid #eee; border-radius: 10px; padding: 15px; margin-bottom: 15px; display: flex; align-items: center; }
        .worker-img { width: 65px; height: 65px; border-radius: 8px; object-fit: cover; margin-right: 15px; }
        .role-tag { font-size: 0.75rem; background: #3498db; color: #fff; padding: 2px 8px; border-radius: 5px; vertical-align: middle; }
    </style>
</head>
<body>
    ...
    <div class="row">
        <div class="col-lg-8">
            <div class="profile-header-card">
                ...
                <div>
                    <h2 class="fw-bold mb-1"><?= htmlspecialchars($userData['name']) ?> <span class="role-tag"><?= ucfirst($role) ?></span></h2>
                    <p class="text-muted small mb-0"><?= htmlspecialchars($introduction) ?></p>
                </div>
                <div class="ms-auto">
                    <button class="btn btn-primary btn-sm px-4" data-bs-toggle="modal" data-bs-target="#editModal">Edit Profile</button>
                </div>
            </div>
    ...
    <div class="modal-dialog">
        <form method="POST" class="modal-content">
            ...
            <div class="modal-body">
                ...
                <?php if($role === 'client'): ?>
                    <div class="mb-3"><label class="form-label small fw-bold">Budget</label><input type="number" name="budget" class="form-control" value="<?= $userData['budget'] ?>"></div>
                <?php endif; ?>
            </div>
