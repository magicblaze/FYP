<?php
// designer/designer_view_reports.php
...
// Verify this order belongs to designer's design
$verify_sql = "SELECT o.orderid, o.ostatus 
               FROM `Order` o
               JOIN Design d ON o.designid = d.designid
               WHERE o.orderid = ? AND d.designerid = ?";
$verify_stmt = mysqli_prepare($mysqli, $verify_sql);
mysqli_stmt_bind_param($verify_stmt, "ii", $order_id, $designer_id);
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['submit_feedback'])) {
    ...
    $designer_feedback = $_POST['designer_feedback'] ?? '';
    
    if (empty($designer_feedback)) {
        $feedback_error = "Please enter your feedback.";
    } else {
        $update_sql = "UPDATE WeeklyConstructionReport 
                       SET designer_feedback = ?, designer_feedback_at = NOW()
                       WHERE orderid = ? AND week_number = ?";
        $update_stmt = mysqli_prepare($mysqli, $update_sql);
        mysqli_stmt_bind_param($update_stmt, "sii", $designer_feedback, $order_id, $week_number);
        ...
        if (mysqli_stmt_execute($update_stmt)) {
            ...
            if ($supplier_id) {
                $notify_sql = "INSERT INTO Notification (user_type, user_id, orderid, message, type, created_at) 
                               VALUES ('supplier', ?, ?, 'Designer has provided feedback on Week $week_number report for Order #$order_id.', 'feedback', NOW())";
...
<head>
    ...
    <style>
        ...
        .designer-feedback-box { background-color: #fff3e0; border-left-color: #fd7e14; }
...
<body>
    ...
    <div class="container container-custom">
        <a href="OrderManager.php" class="btn btn-outline-secondary mb-3">
            ...
            <div class="card-body">
                <div class="row mb-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <span class="badge-status <?= $order_status == 'In construction' ? 'bg-primary text-white' : 'bg-secondary' ?>">
                ...
                <?php else: ?>
                    <?php foreach ($reports as $report): ?>
                        <?php $can_feedback = ($report['status'] == 'submitted' && empty($report['designer_feedback'])); ?>
                        <div class="report-card">
                            ...
                            <div class="report-body" id="report-body-<?= $report['week_number'] ?>" style="display: none;">
                                ...
                                <!-- Display client feedback if any -->
                                ...
                                <?php if (!empty($report['designer_feedback'])): ?>
                                    <div class="feedback-box designer-feedback-box">
                                        <strong><i class="fas fa-user-tie me-1"></i>Your Feedback:</strong>
                                        <p class="mb-1"><?= nl2br(htmlspecialchars($report['designer_feedback'])) ?></p>
                                        <small class="text-muted">Submitted: <?= date('M d, Y H:i', strtotime($report['designer_feedback_at'])) ?></small>
                                    </div>
                                <?php elseif ($can_feedback): ?>
                                    <div class="mt-3 border rounded p-3">
                                        ...
                                        <form method="POST">
                                            ...
                                            <div class="mb-2">
                                                <textarea name="designer_feedback" rows="3" class="form-control" placeholder="Provide your professional feedback on this week's report..."></textarea>
                                            </div>
