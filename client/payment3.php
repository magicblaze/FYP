...
$sql = "SELECT o.orderid, o.odate, o.Requirements, o.ostatus, o.cost, o.designid, o.deposit, o.final_payment,
           ...
           op.design_fee_manager_2nd_pct, op.commission_1st_pct,
...
$design_fee_manager_2nd_pct = isset($order['design_fee_manager_2nd_pct']) ? floatval($order['design_fee_manager_2nd_pct']) : 0;
$commission_1st_pct = isset($order['commission_1st_pct']) ? floatval($order['commission_1st_pct']) : 0;
...
$design_fee_manager_2nd = $total_cost * ($design_fee_manager_2nd_pct / 100);
$commission_1st = $total_cost * ($commission_1st_pct / 100);
...
// Calculate total for this payment stage (Manager 2nd Fee + Commission 1st)
$total_to_pay = $design_fee_manager_2nd + $commission_1st;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['reject_payment'])) {
        ...
        header('Location: payment3.php?orderid=' . $orderid . '&rejected=1');
        exit;
    }
    ...
    if (isset($_POST['proceed_pay'])) {
        ...
        try {
            $milestone = 'Final Design Phase Payment';
            $installment_number = 3;
            $percentage = intval(round($design_fee_manager_2nd_pct + $commission_1st_pct));
            ...
            error_log("payment3.php - orderid: " . $orderid);
            error_log("payment3.php - GET success: " . (isset($_GET['success']) ? 'yes' : 'no'));
            error_log("payment3.php - SESSION payment_success_" . $orderid . ": " . (isset($_SESSION['payment_success_' . $orderid]) ? 'yes' : 'no'));
            error_log("payment3.php - payment_success final: " . ($payment_success ? 'true' : 'false'));
            // Update order status
            ...
            $_SESSION['payment3_success_' . $orderid] = true;
            
            // Redirect to same page with success parameter
            header('Location: payment3.php?orderid=' . $orderid . '&amount=' . $total_to_pay . '&success=1');
            exit;
...
$stage_title = 'Final Design Payment';
?>
<!DOCTYPE html>
<html lang="en">
<head>
        ...
        <div class="card">
            <div class="card-body">
                ...
                <div class="progress-steps">
                    ...
                    <!-- Stage 3: Final Design - Current (Yellow) -->
                    <div class="step <?php 
                ...
                <div class="payment-section">
                    <h5 class="mb-3"><i class="fas fa-check-circle me-2"></i>Final Design Payment</h5>
                    ...
                    <div class="payment-detail">
                        ...
                        <div class="fee-breakdown">
                            <div class="d-flex justify-content-between mb-1">
                                <span>Manager 2nd Design Fee:</span>
                                <span>HK$<?php echo number_format($design_fee_manager_2nd, 2); ?></span>
                            </div>
                            <div class="d-flex justify-content-between">
                                <span>1st Commission:</span>
                                <span>HK$<?php echo number_format($commission_1st, 2); ?></span>
                            </div>
                        </div>
                        
                        <div class="payment-detail-item" style="border-top: 2px solid #27ae60; margin-top: 0.5rem; padding-top: 1rem;">
                            <span class="fw-bold">Total Final Design Payment:</span>
                ...
                <form method="post">
                    <div class="d-flex gap-2">
                        <?php if (!empty($paymentMethodData) && !empty($paymentMethodData['method'])): ?>
                            <?php if ($payment_success): ?>
                                <a href="payment_construction.php?orderid=<?php echo $orderid; ?>" class="btn btn-success">
                                    <i class="fas fa-check-circle me-1"></i>Proceed to Construction Payment
                                </a>
                            <?php else: ?>
