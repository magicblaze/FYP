...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['proceed_pay'])) {
        ...
        try {
            // Update order final payment field
            $u_sql = "UPDATE `Order` SET final_payment = ? WHERE orderid = ? AND clientid = ?";
            ...
            error_log("Payment final error: " . $e->getMessage());
                ...
                <form method="post">
                    <div class="d-flex gap-2">
                        <?php if (!empty($paymentMethodData) && !empty($paymentMethodData['method'])): ?>
                            ...
                            <?php else: ?>
                                <button type="submit" name="proceed_pay" class="btn btn-success" onclick="return confirm('Pay HK$<?php echo number_format($amount, 2); ?> from your budget? Your remaining budget will be HK$<?php echo number_format($current_budget - $amount, 2); ?>');">
