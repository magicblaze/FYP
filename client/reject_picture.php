<?php
// ==============================
// File: client/reject_picture.php
// Handle rejection of designed pictures
// ==============================
...
try {
    ...
    if ($picResult->num_rows === 0) {
        ...
        throw new Exception('You do not have permission to reject this picture');
    }

    $picture = $picResult->fetch_assoc();
    $picStmt->close();

    // Update picture status to rejected
    $updateSql = "UPDATE DesignedPicture SET status = 'rejected', rejection_reason = ?, rejection_date = NOW() WHERE pictureid = ?";
    $updateStmt = $mysqli->prepare($updateSql);
    if (!$updateStmt) {
        throw new Exception('Prepare failed (update): ' . $mysqli->error);
    }

    $updateStmt->bind_param("si", $rejectionReason, $pictureId);
    ...
    echo json_encode([
        'success' => true,
        'message' => 'Picture rejected successfully. Designer can now upload a new picture.'
    ]);
